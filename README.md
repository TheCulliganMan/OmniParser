...
Download a python installer (3.12.x recommended)
https://www.python.org/downloads/windows/

Once python is installed install uv

`pip install uv`

Then clone the repo, and then install environment:
```bash
uv sync
```
...
```python
uv run huggingface_download.py
```
